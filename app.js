      ...
      if (card.card_faces && card.card_faces.length === 2) {
        ...
        return `
          <div class="card">
            ...
            <button class="flipBtn" data-idx="${idx}" type="button" aria-label="Flip card">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-refresh-cw-icon lucide-refresh-cw"><path d="M3 12a9 9 0 0 1 9-9 9.75 9.75 0 0 1 6.74 2.74L21 8"/>
            <path d="M21 3v5h-5"/><path d="M21 12a9 9 0 0 1-9 9 9.75 9.75 0 0 1-6.74-2.74L3 16"/><path d="M8 16H3v5"/></svg>
            </button>
          </div>
        `;
      } else {
      ...
      if (card.card_faces && card.card_faces.length === 2) {
        ...
        btn.addEventListener('click', function(e) {
          ...
          if (showingFront) {
            ...
            img.alt = card.card_faces[1].name;
          } else {
            img.src = card.card_faces[0].image_uris.normal;
            img.alt = card.card_faces[0].name;
          }
    ...
    if (card.card_faces && card.card_faces.length === 2) {
      imageHTML = `
        <div class="flip-card">
          ...
          <button id="detailFlipBtn" type="button" aria-label="Flip card">
